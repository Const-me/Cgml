...
std::wstring Utf8ToWide(absl::string_view input) {
  ...
  if (result > 0)
  {
		// MultiByteToWideChar does not null-terminate the output string. Documented there:
		// https://learn.microsoft.com/en-us/windows/win32/api/stringapiset/nf-stringapiset-multibytetowidechar#remarks
		// Google ignored the documentation, which causes a fail in debug builds with 100% probability
		// Debug heap initializes new memory with a special pattern, which is not zero.
		const wchar_t* const p1 = input_wide.get();
		const wchar_t* const p2 = p1 + result;
		output.assign( p1, p2 );
  }
  return output;
}
