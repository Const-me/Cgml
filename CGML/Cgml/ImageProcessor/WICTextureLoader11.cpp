...
HRESULT DirectX::createTextureFromWIC( _In_ ID3D11Device* d3dDevice,
	_In_ IWICBitmapFrameDecode* frame,
	_Outptr_opt_ ID3D11ShaderResourceView** textureView ) noexcept
{
	constexpr size_t maxsize = 0;
	constexpr D3D11_USAGE usage = D3D11_USAGE_IMMUTABLE;
	constexpr unsigned int bindFlags = D3D11_BIND_SHADER_RESOURCE;
	constexpr unsigned int cpuAccessFlags = 0;
	constexpr unsigned int miscFlags = 0;
	constexpr WIC_LOADER_FLAGS loadFlags = WIC_LOADER_FORCE_RGBA32;

	return CreateTextureFromWIC( d3dDevice, nullptr, frame,
		maxsize,
		usage, bindFlags, cpuAccessFlags, miscFlags,
		loadFlags, nullptr, textureView );
}